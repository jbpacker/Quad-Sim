...
function p = updateOccupencyGrid(angle, range, Tl, R, C, Xmax, Ymax)
    ...
    occupencyGrid(I2, J2) = min(5*occupencyGrid(I2, J2), 1000);
    
    % use bresenham to find all pixels that are between laser and obstacle
   l=bresenham(I1,J1,I2,J2); 
   [l1 l2]=size(l);
   for k=1:length(l)-1 %skip the target pixel
      occupencyGrid(l(k,1),l(k,2)) = 0.1*occupencyGrid(l(k,1),l(k,2)); % free pixels
   end
