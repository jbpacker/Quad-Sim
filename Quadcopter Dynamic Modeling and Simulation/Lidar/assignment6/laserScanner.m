...
% Given a map (occupancy grid with obstacles) with origin at (0,0) and upper corner (Xmax, Ymax), 
...
function p = laserScanner(angleSpan, angleStep, rangeMax, Tl, map, Xmax, Ymax)
    ...
    p = zeros(N, 2); %2 column vector - distance, theta of laser (lidar)
    [R, C] = size(map);
 i=1; 
...
%         l=bresenham(I1,J1,Io,Jo); % for debugging, show scan line
%         for k=1:length(l)
%         map(l(k,1),l(k,2))=1;
%         end
%         imagesc(map);
